...
def get_setting_number(protected, name):
	...
	if not SWRR in ProtectedFuncNumber:
		ProtectedFuncNumber[SWRR] = len(ProtectedFuncNumber)
		ProtectedFuncRef[ProtectedFuncNumber[SWRR]] = name
	return ProtectedFuncNumber[SWRR]
...
def add_loader_code_to_entry(entryFunc, loaderFile, logfilename):
	...
	print >> output, 'int SWRR_flags[' + str(len(ProtectedFuncNumber)) + '];\n'
	print >> output, get_loader_data('SWRR_flags_names', ProtectedFuncNumber, 'SWRR_entry', entryFunc)
...
def get_CP_code(func, protected):
	...
	return "\tif (SWRR_flags[" + str(get_setting_number(protected, name)) + '])\n' + '\t\tgoto ' + get_label(func) + ';\n'
...
def get_CP_combined_code(func, list_protected):
	...
	for protected in list_protected:
		name = protected.replace('/', '_').replace('-', '_').replace('.', '_')
		if cond:
			cond += ' || SWRR_flags[' + str(get_setting_number(name, protected)) + '])'
		else:
			cond = 'SWRR_flags[' + str(get_setting_number(name, protected)) + '])'
	return '\tif (' + cond + ')\n' + '\t\tgoto ' + get_label(func) + ';\n'
...
def add_sec_settings_all(logfilename, settingsfile, checkOnly=False):
	...
	for SWRR in ProtectedFuncNumber:
		ID = ProtectedFuncNumber[SWRR]
		funcName = ProtectedFuncRef[ID]
		if not SWRR in ProtectedByCaller:
			print >>output, SWRR, ID, get_error_return(funcName)[0]
		else:
			print >>output, SWRR, ID, 100
	output.close()
	add_loader_code('Talos_SWRR.c', logfilename)
...
if AddSecuritySetting:
	...
	if Verbose:
		...
		for SWRR in ProtectedFuncNumber:
			...
			funcName = ProtectedFuncRef[ID]
			if not SWRR in ProtectedByCaller:
				print >>sys.stderr, '\t', SWRR, ID, get_error_return(funcName)[0], funcName
			else:
